...
static void create_test_ms(const fs::path& path) {
    ...
    writer.add_antenna({.name = "ANT0", .station = "W01",
                        ...
                        .offset = {0.0, 0.0, 0.0}, .dish_diameter = 25.0,
                        .flag_row = false});
    ...
    writer.add_data_description({.spectral_window_id = 0, .polarization_id = 0,
                                  .flag_row = false});
    writer.add_observation({.telescope_name = "VLA", .observer = "demo", .project = {},
...
int main() {
    ...
    for (std::uint64_t r = 0; r < ms.row_count(); ++r) {
        std::cout << "  row " << r
                  << ": ANT1=" << main_cols.antenna1(r)
                  << " ANT2=" << main_cols.antenna2(r)
                  << " SCAN=" << main_cols.scan_number(r)
    ...
    for (std::uint64_t r = 0; r < ant_cols.row_count(); ++r) {
        std::cout << "  ANT " << r << ": name=" << ant_cols.name(r)
                  << " station=" << ant_cols.station(r)
                  << " dish=" << ant_cols.dish_diameter(r) << " m\n";
    }
    ...
    for (std::size_t i = 0; i < chunks.size(); ++i) {
        ...
        std::cout << "  chunk " << i
                  << ": array=" << chunk.array_id
                  << " field=" << chunk.field_id
    ...
    for (std::size_t i = 0; i < time_chunks.size(); ++i) {
        std::cout << "  chunk " << i
                  << ": time_center=" << time_chunks[i].time_center

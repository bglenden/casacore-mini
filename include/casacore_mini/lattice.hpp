...
template <typename T> class Lattice {
  public:
    ...
    [[nodiscard]] std::int64_t nelements() const {
    ...
    [[nodiscard]] virtual bool is_paged() const {
        return false;
    }
...
///
template <typename T> class MaskedLattice : public Lattice<T> {
  public:
    /// Whether the lattice has a pixel mask.
    [[nodiscard]] virtual bool is_masked() const {
        return has_pixel_mask();
    }
    ...
    [[nodiscard]] virtual LatticeArray<bool> get_mask_slice(const Slicer& slicer) const = 0;
};
...
/// @endcode
///
template <typename T> class ArrayLattice : public MaskedLattice<T> {
  public:
    ...
    [[nodiscard]] const LatticeArray<T>& array() const {
        return data_;
    }
    [[nodiscard]] LatticeArray<T>& array() {
        return data_;
    }
...
///
template <typename T> class PagedArray : public MaskedLattice<T> {
  public:
    ...
    explicit PagedArray(const std::filesystem::path& path, bool writable = false);
...
///
template <typename T> class SubLattice : public MaskedLattice<T> {
  public:
...
/// @endcode
///
template <typename T> class TempLattice : public MaskedLattice<T> {
  public:
...
///
template <typename T> class LatticeConcat : public MaskedLattice<T> {
  public:
    ...
    [[nodiscard]] std::size_t nlattices() const {
        return lattices_.size();
    }
  ...
  private:
    ...
    [[nodiscard]] std::pair<std::size_t, std::int64_t> locate(std::int64_t global_pos) const;
...
/// @endcode
///
template <typename T> class RebinLattice : public MaskedLattice<T> {
  public:
    ...
    [[nodiscard]] const IPosition& factors() const {
        return factors_;
    }
...
template <typename T> class LatticeIterator {
  public:
    ...
    [[nodiscard]] const IPosition& position() const {
        return position_;
    }
...
template <typename T> void Lattice<T>::set(const T& value) {
    auto data = LatticeArray<T>(shape(), value);
    put(data);
}

template <typename T> void Lattice<T>::apply(const std::function<T(T)>& func) {
    auto data = get();
    data.make_unique();
...
template <typename T> ArrayLattice<T>::ArrayLattice(IPosition shape) : data_(std::move(shape)) {}

template <typename T>
ArrayLattice<T>::ArrayLattice(IPosition shape, T value) : data_(std::move(shape), value) {}

template <typename T>
ArrayLattice<T>::ArrayLattice(LatticeArray<T> data) : data_(std::move(data)) {}

template <typename T> LatticeArray<bool> ArrayLattice<T>::get_mask() const {
    return LatticeArray<bool>(data_.shape(), true);
}

template <typename T>
LatticeArray<bool> ArrayLattice<T>::get_mask_slice(const Slicer& slicer) const {
    return LatticeArray<bool>(IPosition(slicer.length()), true);
}

template <typename T> LatticeArray<T> ArrayLattice<T>::get_slice(const Slicer& slicer) const {
    return data_.get_slice(slicer);
}

template <typename T> T ArrayLattice<T>::get_at(const IPosition& where) const {
    return data_.at(where);
}

template <typename T> void ArrayLattice<T>::put(const LatticeArray<T>& data) {
    data_ = data;
    data_.make_unique();
}

template <typename T>
void ArrayLattice<T>::put_slice(const LatticeArray<T>& data, const Slicer& slicer) {
    data_.make_unique();
    data_.put_slice(data, slicer);
}

template <typename T> void ArrayLattice<T>::put_at(const T& value, const IPosition& where) {
    data_.make_unique();
...
SubLattice<T>::SubLattice(Lattice<T>& parent, Slicer slicer)
    : parent_rw_(&parent), parent_(&parent), region_(std::move(slicer)),
      sub_shape_(region_.length()), writable_(parent.is_writable()) {
    validate_slicer(region_, parent.shape());
}

template <typename T>
SubLattice<T>::SubLattice(const Lattice<T>& parent, Slicer slicer)
    : parent_(&parent), region_(std::move(slicer)), sub_shape_(region_.length()), writable_(false) {
    validate_slicer(region_, parent.shape());
}

template <typename T> LatticeArray<bool> SubLattice<T>::get_mask() const {
    return LatticeArray<bool>(sub_shape_, true);
}

template <typename T> LatticeArray<bool> SubLattice<T>::get_mask_slice(const Slicer& slicer) const {
    return LatticeArray<bool>(IPosition(slicer.length()), true);
}

template <typename T> LatticeArray<T> SubLattice<T>::get() const {
    return parent_->get_slice(region_);
}

template <typename T> LatticeArray<T> SubLattice<T>::get_slice(const Slicer& slicer) const {
    return parent_->get_slice(to_parent_slicer(slicer));
}

template <typename T> T SubLattice<T>::get_at(const IPosition& where) const {
    return parent_->get_at(to_parent_index(where));
}

template <typename T> void SubLattice<T>::put(const LatticeArray<T>& data) {
    if (!writable_)
        throw std::runtime_error("SubLattice is read-only");
    parent_rw_->put_slice(data, region_);
}

template <typename T>
void SubLattice<T>::put_slice(const LatticeArray<T>& data, const Slicer& slicer) {
    if (!writable_)
        throw std::runtime_error("SubLattice is read-only");
    parent_rw_->put_slice(data, to_parent_slicer(slicer));
}

template <typename T> void SubLattice<T>::put_at(const T& value, const IPosition& where) {
    if (!writable_)
        throw std::runtime_error("SubLattice is read-only");
    parent_rw_->put_at(value, to_parent_index(where));
}

template <typename T> IPosition SubLattice<T>::to_parent_index(const IPosition& local) const {
...
template <typename T> Slicer SubLattice<T>::to_parent_slicer(const Slicer& local) const {
...
template <typename T> TempLattice<T>::TempLattice(IPosition shape, std::size_t max_memory_bytes) {
    ...
    if (total_bytes <= max_memory_bytes) {
        ...
        auto name =
            ...
            std::to_string(std::hash<std::string>{}(
                shape.to_string() + std::to_string(reinterpret_cast<std::uintptr_t>(this))));
        auto path = tmp_dir / name;
        impl_ = std::make_unique<PagedArray<T>>(std::move(shape), path);
    }
}

template <typename T> LatticeArray<bool> TempLattice<T>::get_mask() const {
    return LatticeArray<bool>(impl_->shape(), true);
}

template <typename T>
LatticeArray<bool> TempLattice<T>::get_mask_slice(const Slicer& slicer) const {
    return LatticeArray<bool>(IPosition(slicer.length()), true);
}
...
LatticeIterator<T>::LatticeIterator(Lattice<T>& lattice, IPosition cursor_shape)
    : lattice_rw_(&lattice), lattice_(&lattice), requested_shape_(std::move(cursor_shape)),
      position_(lattice.shape().ndim(), 0), writable_(true) {}

template <typename T>
LatticeIterator<T>::LatticeIterator(const Lattice<T>& lattice, IPosition cursor_shape)
    : lattice_(&lattice), requested_shape_(std::move(cursor_shape)),
      position_(lattice.shape().ndim(), 0), writable_(false) {}

template <typename T> void LatticeIterator<T>::reset() {
    for (std::size_t i = 0; i < position_.ndim(); ++i)
        position_[i] = 0;
}

template <typename T> LatticeIterator<T>& LatticeIterator<T>::operator++() {
    const auto& shape = lattice_->shape();
    for (std::size_t d = 0; d < position_.ndim(); ++d) {
        position_[d] += requested_shape_[d];
        if (position_[d] < shape[d])
            return *this;
        position_[d] = 0;
    }
...
template <typename T> bool LatticeIterator<T>::at_end() const {
    const auto& shape = lattice_->shape();
    for (std::size_t d = 0; d < position_.ndim(); ++d) {
        if (position_[d] >= shape[d])
            return true;
...
template <typename T> IPosition LatticeIterator<T>::cursor_shape() const {
    const auto& shape = lattice_->shape();
...
template <typename T> LatticeArray<T> LatticeIterator<T>::cursor() const {
    auto cs = cursor_shape();
    return lattice_->get_slice(Slicer(IPosition(position_), cs));
}

template <typename T> void LatticeIterator<T>::write_cursor(const LatticeArray<T>& data) {
    if (!writable_)
        throw std::runtime_error("LatticeIterator is read-only");
    auto cs = cursor_shape();
...
template <typename T>
LatticeConcat<T>::LatticeConcat(std::vector<const Lattice<T>*> lattices, std::size_t axis)
    ...
    for (std::size_t i = 1; i < lattices_.size(); ++i) {
        ...
        for (std::size_t d = 0; d < ndim; ++d) {
            if (d != axis_ && s[d] != shape_[d]) {
                throw std::runtime_error("LatticeConcat: mismatched shape on non-concat axis");
            }
        }
...
template <typename T> LatticeArray<bool> LatticeConcat<T>::get_mask() const {
    return LatticeArray<bool>(shape_, true);
}

template <typename T>
LatticeArray<bool> LatticeConcat<T>::get_mask_slice(const Slicer& slicer) const {
    return LatticeArray<bool>(IPosition(slicer.length()), true);
}

template <typename T>
std::pair<std::size_t, std::int64_t> LatticeConcat<T>::locate(std::int64_t global_pos) const {
...
template <typename T> LatticeArray<T> LatticeConcat<T>::get() const {
...
template <typename T> LatticeArray<T> LatticeConcat<T>::get_slice(const Slicer& slicer) const {
    auto full = get();
    return full.get_slice(slicer);
}

template <typename T> T LatticeConcat<T>::get_at(const IPosition& where) const {
...
template <typename T> void LatticeConcat<T>::put(const LatticeArray<T>& /*data*/) {
    throw std::runtime_error("LatticeConcat is read-only");
}

template <typename T>
void LatticeConcat<T>::put_slice(const LatticeArray<T>& /*data*/, const Slicer& /*slicer*/) {
    throw std::runtime_error("LatticeConcat is read-only");
}
...
template <typename T> LatticeArray<bool> RebinLattice<T>::get_mask() const {
    return LatticeArray<bool>(shape_, true);
}

template <typename T>
LatticeArray<bool> RebinLattice<T>::get_mask_slice(const Slicer& slicer) const {
    return LatticeArray<bool>(IPosition(slicer.length()), true);
}

template <typename T> LatticeArray<T> RebinLattice<T>::get() const {
...
template <typename T> LatticeArray<T> RebinLattice<T>::get_slice(const Slicer& slicer) const {
    auto full = get();
    return full.get_slice(slicer);
}

template <typename T> T RebinLattice<T>::get_at(const IPosition& where) const {
    const auto ndim = shape_.ndim();
    const auto& ps = parent_->shape();
...
template <typename T> void RebinLattice<T>::put(const LatticeArray<T>& /*data*/) {
    throw std::runtime_error("RebinLattice is read-only");
}

template <typename T>
void RebinLattice<T>::put_slice(const LatticeArray<T>& /*data*/, const Slicer& /*slicer*/) {
    throw std::runtime_error("RebinLattice is read-only");
}

template <typename T> void RebinLattice<T>::put_at(const T& /*value*/, const IPosition& /*where*/) {
    throw std::runtime_error("RebinLattice is read-only");
}

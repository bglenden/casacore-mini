...
///
///   - A restoring beam described by major axis, minor axis, and position
///        angle (all in radians).  A beam is considered absent when either
///        `beam_major_rad` or `beam_minor_rad` is zero.
///   - An optional object name string (e.g., "NGC 1234").
///   - An image type string (e.g., "Intensity", "Beam", "SpectralIndex").
///
///
/// The struct is serialised to and from a `Record` so it can be
/// stored as a table keyword inside a `PagedImage` on disk.  The
/// keyword name used by `PagedImage` is `"imageinfo"`.
///
/// @warning
...
/// arcseconds before constructing an `ImageInfo`.
///
///
///
/// @par Example
...
///
/// @par Prerequisites
///   - MaskedLattice<T> — pixel data + boolean mask interface
///   - CoordinateSystem — world-coordinate description of each pixel axis
///   - ImageInfo — restoring beam and object metadata
///
///
///
...
///
///   - A CoordinateSystem describing each pixel axis in world coordinates
///   - A brightness unit string (e.g. "Jy/beam", "K")
///   - An ImageInfo record holding the restoring beam, object name, and type
///   - A miscellaneous keyword Record for user-defined metadata
///
///
...
///
/// Mask convention: a pixel mask value of `true` means the
/// pixel is valid; `false` means it is flagged.
///
///
/// @par Example
...
///
template <typename T> class ImageInterface : public MaskedLattice<T> {
  public:
    ...
    [[nodiscard]] virtual bool is_persistent() const {
        return false;
    }
};
...
///
/// @par Prerequisites
///   - ImageInterface<T> — abstract image base class
///   - CoordinateSystem — axis description serialized as a table keyword
///   - Table — casacore on-disk table (read/write access)
///
///
///
...
/// The table layout is:
///
///   - One array column "map" backed by TiledShapeStMan
///   - Table keyword "coords"    → CoordinateSystem Record
///   - Table keyword "imageinfo" → ImageInfo Record
///   - Table keyword "miscinfo"  → miscellaneous Record
...
/// `make_mask()`.  The default mask is the one last registered
/// via `make_mask(..., set_default=true)`.
///
/// Metadata is loaded into memory on construction and flushed back to
/// the table either on `flush()` or on destruction.  Pixel data
/// is read and written directly through the table column without buffering.
///
/// @warning
/// Opening the same table path from two `PagedImage` instances
...
/// any conversion step.
///
template <typename T> class PagedImage : public ImageInterface<T> {
  public:
    /// Create a new image on disk.
    PagedImage(IPosition shape, CoordinateSystem cs, const std::filesystem::path& path);
    /// Open an existing image from disk (read-only by default).
    explicit PagedImage(const std::filesystem::path& path, bool writable = false);

    // -- Lattice interface --
    [[nodiscard]] const IPosition& shape() const override;
    [[nodiscard]] bool is_writable() const override;
    [[nodiscard]] bool is_paged() const override {
        return true;
    }
    [[nodiscard]] bool has_pixel_mask() const override;
    [[nodiscard]] LatticeArray<bool> get_mask() const override;
    [[nodiscard]] LatticeArray<bool> get_mask_slice(const Slicer& slicer) const override;
    ...
    void make_mask(const std::string& mask_name, const LatticeArray<bool>& mask_data,
...
/// @par Prerequisites
///   - ImageInterface<T> — abstract image base class
///   - LatticeArray<T> — in-memory pixel storage
///
///
///
/// TempImage holds all pixel data in a `LatticeArray<T>` residing
/// entirely in RAM.  It satisfies the full `ImageInterface<T>`
/// contract — coordinate system, units, ImageInfo, and misc keywords — but
/// never touches the filesystem.
///
/// Because storage is in-memory, `is_persistent()` returns
/// `false` and `is_paged()` returns `false`.
/// The image is always writable.
///
/// An optional boolean pixel mask can be attached via `attach_mask()`.
/// Until a mask is attached, `has_pixel_mask()` returns
/// `false` and all pixels are treated as valid.
///
...
/// @endcode
///
template <typename T> class TempImage : public ImageInterface<T> {
  public:
    ...
    /// `has_pixel_mask()` returns `true`.  Calling
    /// `attach_mask()` a second time replaces the previous mask.
...
/// @par Prerequisites
///   - ImageInterface<T> — abstract image base class
///   - Slicer — describes the sub-region (start, length, stride per axis)
///   - CoordinateSystem — adjusted to reflect reference-pixel shift
///
///
///
...
/// `ImageInterface<T>&` is writable; one constructed from a
/// `const ImageInterface<T>&` is read-only.  Attempting any write
/// operation on a read-only SubImage throws `std::runtime_error`.
///
/// @warning
...
/// @endcode
///
template <typename T> class SubImage : public ImageInterface<T> {
  public:
...
///
/// @par Prerequisites
///   - ImageInfo — single restoring beam for non-spectral images
///
///
///
/// ImageBeamSet generalises the single restoring beam in `ImageInfo`
...
///
/// The storage is a `std::vector<std::vector<Beam>>` where the outer
/// index is channel and the inner index is Stokes.  A single-beam set is
/// represented as a 1x1 grid and can be tested with `is_single()`.
///
/// Serialisation to and from a `Record` is provided for storage as
/// a table keyword inside a `PagedImage`.
///
/// @warning
/// All beam parameters (major, minor, position angle) are stored in radians
/// to match `ImageInfo` conventions.
///
///
///
/// @par Example
...
///
///
/// In casacore, a `PagedImage` can store multiple named boolean
/// masks as subtables.  MaskSpecifier encapsulates the choice of which mask
/// to activate.  An empty name (`""`) selects the image's current
/// default mask, which is determined by the `"logtable"` keyword
/// inside the table.
///
...
struct MaskSpecifier {
    ...
    explicit MaskSpecifier(std::string mask_name) : name(std::move(mask_name)) {}
};
...
/// @par Prerequisites
///   - ImageInterface<T> — abstract image base class
///   - LatticeArray<T> — in-memory storage for evaluated pixel data
///   - CoordinateSystem — inherited from the reference image
///
///
///
/// ImageExpr wraps a pre-evaluated `LatticeArray<T>` — the result
/// of computing a lattice arithmetic expression — as a fully-fledged
/// `ImageInterface<T>`.  The coordinate system, units, and
...
/// storage, all write operations (`put()`, `put_slice()`,
/// `put_at()`) throw `std::runtime_error`.
/// `is_writable()` always returns `false`.
///
/// The optional `expr_text` string stores the human-readable
/// expression (e.g., `"2.0 * img1 - img2"`) for informational
/// purposes and is returned by `name()` as
/// `"ImageExpr(expr_text)"`.
///
/// All pixels are treated as valid — `has_pixel_mask()` returns
/// `false` and `get_mask()` returns an all-`true`
/// array.
///
/// @warning
...
/// @endcode
///
template <typename T> class ImageExpr : public ImageInterface<T> {
  public:
    /// Construct from evaluated data and a reference coordinate system.
    ImageExpr(LatticeArray<T> data, CoordinateSystem cs, std::string expr_text = "");
...
/// @par Prerequisites
///   - ImageInterface<T> — abstract image base class
///   - LatticeConcat<T> — underlying pixel concatenation mechanism
///   - CoordinateSystem — inherited from the first component image
///
///
///
/// ImageConcat presents a set of images joined end-to-end along one pixel
/// axis as a single `ImageInterface<T>`.  The underlying pixel data
/// is collected into a `LatticeConcat<T>` eagerly at construction
...
/// All write operations throw `std::runtime_error`;
/// `is_writable()` always returns `false`.
///
/// Masks are not propagated from component images — `has_pixel_mask()`
/// returns `false` and all pixels are treated as valid.  If mask
...
/// @endcode
///
template <typename T> class ImageConcat : public ImageInterface<T> {
  public:
    /// Construct from a list of images and the axis to concatenate on.
    ImageConcat(std::vector<const ImageInterface<T>*> images, std::size_t axis);

    // -- Lattice interface --
    [[nodiscard]] const IPosition& shape() const override {
        return concat_.shape();
    }
    [[nodiscard]] bool is_writable() const override {
        return false;
    }
    ...
    [[nodiscard]] std::size_t nimages() const {
        return images_.size();
    }

  private:
    static LatticeConcat<T> build_concat(const std::vector<const ImageInterface<T>*>& images,
                                         std::size_t axis);
...
/// `print()`.  It reports the image name, shape, brightness units,
/// write status, and — when present — restoring beam parameters and object
/// name.
///
/// Output is written to any `std::ostream` (defaulting to
/// `std::cout`), making it easy to redirect to a log file or
/// string stream.
///
///
/// @par Example
...
struct ImageSummary {
    ...
    template <typename T>
    static void print(const ImageInterface<T>& img, std::ostream& os = std::cout);
};
...
/// zero-initialized `ImageStats` when the image is empty.
template <typename T> ImageStats<T> statistics(const ImageInterface<T>& img);
...
template <typename T> void copy_image(const ImageInterface<T>& src, ImageInterface<T>& dst);
...
/// source.  The computation is performed in `double` precision and
/// the result is cast back to `T`.
template <typename T>
void scale_image(const ImageInterface<T>& src, ImageInterface<T>& dst, double scale,
                 double offset = 0.0);
...
///
///
/// `image_regrid()` reprojects the source image `src`
/// onto the coordinate grid defined by the destination image `dst`
/// using nearest-neighbor interpolation.
///
/// The algorithm iterates over every output pixel position, converts it to
/// world coordinates via `dst`'s coordinate system, then maps the
/// world coordinate back to a pixel position in `src`'s coordinate
...
///
/// This is a simplified analogue of casacore's `ImageRegrid` class.
/// It handles axis permutations and grid changes that can be expressed through
/// the coordinate systems of `src` and `dst`, but does not
/// perform:
///
///   - Higher-order interpolation (bilinear, bicubic, Lanczos).
///   - Spectral or polarization axis regridding beyond coordinate mapping.
///   - Flux conservation (division by Jacobian of the coordinate transform).
///
///
...
/// @endcode
///
template <typename T> void image_regrid(const ImageInterface<T>& src, ImageInterface<T>& dst);
...
template <typename T> LatticeArray<bool> TempImage<T>::get_mask() const {
    if (has_mask_)
        return mask_;
    return LatticeArray<bool>(data_.shape(), true);
}

template <typename T> LatticeArray<bool> TempImage<T>::get_mask_slice(const Slicer& slicer) const {
    if (has_mask_)
        return mask_.get_slice(slicer);
    return LatticeArray<bool>(IPosition(slicer.length()), true);
}

template <typename T> void TempImage<T>::put(const LatticeArray<T>& data) {
    data_ = data;
    data_.make_unique();
}

template <typename T>
void TempImage<T>::put_slice(const LatticeArray<T>& data, const Slicer& slicer) {
    data_.make_unique();
    data_.put_slice(data, slicer);
}

template <typename T> void TempImage<T>::put_at(const T& value, const IPosition& where) {
    data_.make_unique();
    data_.put(where, value);
}

template <typename T> void TempImage<T>::attach_mask(LatticeArray<bool> mask) {
    mask_ = std::move(mask);
    has_mask_ = true;
}
...
SubImage<T>::SubImage(ImageInterface<T>& parent, Slicer slicer)
    : parent_rw_(&parent), parent_(&parent), region_(std::move(slicer)),
      sub_shape_(region_.length()), cs_(CoordinateSystem::restore(parent.coordinates().save())),
...
SubImage<T>::SubImage(const ImageInterface<T>& parent, Slicer slicer)
    : parent_(&parent), region_(std::move(slicer)), sub_shape_(region_.length()),
      cs_(CoordinateSystem::restore(parent.coordinates().save())), writable_(false) {
    validate_slicer(region_, parent.shape());
}

template <typename T> LatticeArray<bool> SubImage<T>::get_mask() const {
    if (!parent_->has_pixel_mask())
        return LatticeArray<bool>(sub_shape_, true);
    return parent_->get_mask_slice(region_);
}

template <typename T> LatticeArray<bool> SubImage<T>::get_mask_slice(const Slicer& slicer) const {
    if (!parent_->has_pixel_mask())
        return LatticeArray<bool>(IPosition(slicer.length()), true);
    return parent_->get_mask_slice(to_parent_slicer(slicer));
}

template <typename T> LatticeArray<T> SubImage<T>::get() const {
    return parent_->get_slice(region_);
}

template <typename T> LatticeArray<T> SubImage<T>::get_slice(const Slicer& slicer) const {
    return parent_->get_slice(to_parent_slicer(slicer));
}

template <typename T> T SubImage<T>::get_at(const IPosition& where) const {
    return parent_->get_at(to_parent_index(where));
}

template <typename T> void SubImage<T>::put(const LatticeArray<T>& data) {
    if (!writable_)
        throw std::runtime_error("SubImage is read-only");
    parent_rw_->put_slice(data, region_);
}

template <typename T>
void SubImage<T>::put_slice(const LatticeArray<T>& data, const Slicer& slicer) {
    if (!writable_)
        throw std::runtime_error("SubImage is read-only");
    parent_rw_->put_slice(data, to_parent_slicer(slicer));
}

template <typename T> void SubImage<T>::put_at(const T& value, const IPosition& where) {
    if (!writable_)
        throw std::runtime_error("SubImage is read-only");
    parent_rw_->put_at(value, to_parent_index(where));
}

template <typename T> void SubImage<T>::set_units(std::string unit_str) {
    if (!writable_)
        throw std::runtime_error("SubImage is read-only");
    parent_rw_->set_units(std::move(unit_str));
}

template <typename T> void SubImage<T>::set_image_info(ImageInfo info) {
    if (!writable_)
        throw std::runtime_error("SubImage is read-only");
    parent_rw_->set_image_info(std::move(info));
}

template <typename T> void SubImage<T>::set_misc_info(Record info) {
    if (!writable_)
        throw std::runtime_error("SubImage is read-only");
    parent_rw_->set_misc_info(std::move(info));
}

template <typename T> std::string SubImage<T>::name() const {
    return parent_->name() + " (sub)";
}

template <typename T> IPosition SubImage<T>::to_parent_index(const IPosition& local) const {
...
template <typename T> Slicer SubImage<T>::to_parent_slicer(const Slicer& local) const {
...
template <typename T>
ImageExpr<T>::ImageExpr(LatticeArray<T> data, CoordinateSystem cs, std::string expr_text)
    : data_(std::move(data)), cs_(std::move(cs)), expr_text_(std::move(expr_text)) {}

template <typename T> LatticeArray<bool> ImageExpr<T>::get_mask() const {
    return LatticeArray<bool>(data_.shape(), true);
}

template <typename T> LatticeArray<bool> ImageExpr<T>::get_mask_slice(const Slicer& slicer) const {
    return LatticeArray<bool>(IPosition(slicer.length()), true);
}

template <typename T> void ImageExpr<T>::put(const LatticeArray<T>& /*data*/) {
    throw std::runtime_error("ImageExpr is read-only");
}

template <typename T>
void ImageExpr<T>::put_slice(const LatticeArray<T>& /*data*/, const Slicer& /*slicer*/) {
    throw std::runtime_error("ImageExpr is read-only");
}

template <typename T> void ImageExpr<T>::put_at(const T& /*value*/, const IPosition& /*where*/) {
    throw std::runtime_error("ImageExpr is read-only");
}

template <typename T> std::string ImageExpr<T>::name() const {
...
template <typename T>
LatticeConcat<T> ImageConcat<T>::build_concat(const std::vector<const ImageInterface<T>*>& images,
                                              std::size_t axis) {
...
template <typename T>
ImageConcat<T>::ImageConcat(std::vector<const ImageInterface<T>*> images, std::size_t axis)
    : images_(std::move(images)), concat_(build_concat(images_, axis)) {
    if (!images_.empty()) {
...
template <typename T> LatticeArray<bool> ImageConcat<T>::get_mask() const {
    return LatticeArray<bool>(concat_.shape(), true);
}

template <typename T>
LatticeArray<bool> ImageConcat<T>::get_mask_slice(const Slicer& slicer) const {
    return LatticeArray<bool>(IPosition(slicer.length()), true);
}

template <typename T> void ImageConcat<T>::put(const LatticeArray<T>& /*data*/) {
    throw std::runtime_error("ImageConcat is read-only");
}

template <typename T>
void ImageConcat<T>::put_slice(const LatticeArray<T>& /*data*/, const Slicer& /*slicer*/) {
    throw std::runtime_error("ImageConcat is read-only");
}

template <typename T> void ImageConcat<T>::put_at(const T& /*value*/, const IPosition& /*where*/) {
    throw std::runtime_error("ImageConcat is read-only");
}

template <typename T> std::string ImageConcat<T>::name() const {
    return "ImageConcat";
}

// ── ImageSummary implementation ───────────────────────────────────────

template <typename T> void ImageSummary::print(const ImageInterface<T>& img, std::ostream& os) {
    os << "Image: " << img.name() << "\n";
    ...
    if (info.has_beam()) {
        os << "Beam: " << info.beam_major_rad << " x " << info.beam_minor_rad << " rad, PA "
...
template <typename T> void copy_image(const ImageInterface<T>& src, ImageInterface<T>& dst) {
    dst.put(src.get());
    dst.set_coordinates(CoordinateSystem::restore(src.coordinates().save()));
...
void scale_image(const ImageInterface<T>& src, ImageInterface<T>& dst, double scale,
    ...
    auto n = data.nelements();
    for (std::size_t i = 0; i < n; ++i) {
        result.mutable_data()[i] = static_cast<T>(static_cast<double>(data[i]) * scale + offset);
    }
    dst.put(result);
}

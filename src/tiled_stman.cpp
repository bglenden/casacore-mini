...
void TiledStManReader::open(const std::string_view table_dir, const std::size_t sm_index,
    ...
    for (const auto& ci : columns_) {
        ...
        col_is_bool.push_back(ci.data_type == DataType::tp_bool);
    }
    ...
    if (have_header && !tsm_header.cubes.empty()) {
        ...
        for (std::size_t cube_index = 0; cube_index < tsm_header.cubes.size(); ++cube_index) {
            ...
            for (std::size_t i = 0; i < tile_layout_.sorted_col_indices.size(); ++i) {
                ...
                const bool is_bool = columns_[sorted_idx].data_type == DataType::tp_bool;
                offset += col_tile_bytes_for_layout(layout.tile_pixels,
                              columns_[sorted_idx].element_size, is_bool);
            }
            layout.bucket_size = offset;
...
std::vector<std::uint8_t> TiledStManReader::read_raw_cell(const std::string_view col_name,
    ...
    if (full_cell_per_tile) {
        ...
        if (is_bool_col) {
            // Bool data is bit-packed on disk: (elem_count+7)/8 bytes.
            const auto disk_bytes = (elem_count + 7) / 8;
            if (byte_offset > file_data->size() || disk_bytes > file_data->size() - byte_offset) {
                throw std::runtime_error("TSM0 read beyond file end");
            }
            const auto* src = file_data->data() + byte_offset;
            for (std::uint64_t i = 0; i < elem_count; ++i) {
                result[i] = (src[i / 8] >> (i % 8)) & 1;
            }
        } else {
    ...
    while (!done) {
        ...
        for (std::uint64_t p = 0; p < chunk_pixels; ++p) {
            ...
            if (is_bool_col) {
                // Bool data is bit-packed: extract single bit.
                const auto bit_pos = row_in_tile * tile_cell_pixels + tile_cell_linear;
                const auto src_byte = tile_base + bit_pos / 8;
                const auto src_bit = bit_pos % 8;
                ...
                result[out_cell_linear] = (file_data->at(src_byte) >> src_bit) & 1;
            } else {
                const auto src =
